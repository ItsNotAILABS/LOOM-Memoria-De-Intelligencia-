...
module {
    ...
    /// Route an intelligence query through the organism
    /// The organism decides which neural cluster should handle it
    public func routeIntelligence(
        ...
        // Find the best neural cluster for this query
        var bestCluster : ?NeuralCluster = null;
        var bestScore : Float = 0.0;
        
        for (cluster in state.neural_clusters.vals()) {
            let score = scoreClusterForQuery(cluster, query);
            if (score > bestScore) {
                bestScore := score;
                bestCluster := ?cluster;
            };
        ...
        let response = switch (bestCluster) {
            case (?cluster) {
                "Routed to " # cluster.cluster_id # " (score: " # Float.toText(bestScore) # ")"
            };
            case (null) {
