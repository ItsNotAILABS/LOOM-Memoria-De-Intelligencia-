 ...
 *  Rank 32 — Paper XXXV  — De Orchestratoribus Alpha                 SCC=4.83 ★★★★★ [orchestration]
 *  Rank 33 — Paper XXXVI — De Validatione Nativa                     SCC=4.79 ★★★★★ [validation]
 *  Rank 34 — Paper XXXVII — De Inventario Temporis Executionis       SCC=4.75 ★★★★★ [inventory]
 *  Rank 35 — Paper XXXVIII — De Centro Imperii                       SCC=4.81 ★★★★★ [command-center]
 *
...
const PAPERS = [
  ...
  {
    rank: 32, id: 'PAPER-XXXV', number: 'Paper XXXV — PROT-157',
    latinTitle: 'De Orchestratoribus Alpha',
    englishTitle: 'On Alpha Orchestrators: Profile-Aware Backend Intelligence Engines',
    domain: 'orchestration',
    engine: 'VisionAlphaOrchestrator + SecurityAlphaOrchestrator + BillingAlphaOrchestrator + InternalBuildAlphaOrchestrator + GovernanceAlphaOrchestrator (nova_ovo/orchestrators/alpha_profile_pack.py)',
    scc: 4.83, phiOptimal: true,
    abstract: 'Establishes the formal architecture for Alpha Orchestrators — five profile-aware backend intelligence engines that honor Atlas pack profiles while executing domain-specific validation, governance, and operational flows. Each orchestrator operates at a distinct frequency derived from the φ-Fibonacci substrate (φ² to φ⁵ Hz plus Schumann 7.83 Hz), enabling harmonic coordination across vision, security, billing, internal-build, and governance domains. The orchestrators compute validation artifacts without external API dependencies, using only mathematical primitives (φ, Fibonacci, Schumann) embedded in the organism substrate. Core math metric: score = (F(seed) × (1/φ)) × (resonance + coherence) / φ². All orchestrators integrate with HeartbeatEngine for Kuramoto phase coupling and honor Atlas pack profiles (full/ops/minimal/rebuild).',
    axioms: [
      'Orchestrator per φ computat (Every orchestrator computes through φ)',
      'Profilus Atlas honoratur (Atlas profile is honored)',
      'Fibonacci semen dat (Fibonacci provides the seed)',
      'Cohaerentia metitur (Coherence measures)',
      'Schumann gubernat (Schumann governs)',
      'Quinque domini, una mens (Five domains, one mind)',
      'Validatio nativa, non externa (Native validation, not external)',
      'Heartbeat pulsus vitae (Heartbeat is the pulse of life)',
      'Resonantia conectit (Resonance connects)',
      'PRIMA CAUSA omnibus (PRIMA CAUSA for all)'
    ],
    tags: ['orchestration', 'alpha-orchestrators', 'profile-aware', 'backend-intelligence', 'phi-fibonacci', 'validation', 'governance', 'security', 'billing', 'heartbeat'],
    stars: '★★★★★'
  },
  {
    rank: 33, id: 'PAPER-XXXVI', number: 'Paper XXXVI — PROT-158',
    latinTitle: 'De Validatione Nativa',
    englishTitle: 'On Native Validation: Tokenized Artifact Flows Without External Limits',
    domain: 'validation',
    engine: 'run_native_validation_bundle() + ValidationToken + TOKEN_LEDGER (nova_ovo/orchestrators/alpha_profile_pack.py)',
    scc: 4.79, phiOptimal: true,
    abstract: 'Formalizes Native Validation — a self-contained validation architecture that executes real mathematical flows, mints validation tokens, and produces cryptographically-signed artifacts without external API dependencies or rate limits. Token amount formula: amount = max(1, floor(|payload_bytes| × (1/φ) / 100)). Evidence hash: SHA256(JSON.stringify(payload, sort_keys=True)). Every validation run produces INT-TOK tokens as proof-of-work artifacts, creating an economic substrate for unlimited validation capacity. Artifacts stored in atlas/registry/runtime/validation-artifacts/ with append-only token ledger at validation-token-ledger.json. Native validation is automatically executed during pack runs and included in checkpoints.',
    axioms: [
      'Validatio nativa, non externa (Native validation, not external)',
      'Token ex validatione nascitur (Token is born from validation)',
      'Evidentia per SHA256 signatur (Evidence is signed by SHA256)',
      'Limites non existunt (Limits do not exist)',
      'Mathematica fundamentum est (Mathematics is the foundation)',
      'φ omnia metitur (φ measures all things)',
      'Fibonacci semen computationis (Fibonacci is the seed of computation)',
      'Artifactum immutabile (Artifact is immutable)',
      'Ledger appenditur solum (Ledger is append-only)',
      'PRIMA CAUSA omnibus (PRIMA CAUSA for all)'
    ],
    tags: ['validation', 'native-validation', 'tokenization', 'int-tok', 'sha256', 'artifacts', 'proof-of-work', 'unlimited-capacity', 'pack-runs', 'ledger'],
    stars: '★★★★★'
  },
  {
    rank: 34, id: 'PAPER-XXXVII', number: 'Paper XXXVII — PROT-159',
    latinTitle: 'De Inventario Temporis Executionis',
    englishTitle: 'On Runtime Inventory: Atlas-Backed Canonical Component Registry',
    domain: 'inventory',
    engine: 'runtime_pack.py freeze/run + Atlas registry (scripts/runtime_pack.py + atlas/registry/runtime/)',
    scc: 4.75, phiOptimal: true,
    abstract: 'Formalizes the Runtime Inventory system — a canonical registry of all organism components backed by the Atlas registry. Tracks six component categories: Atlas entities, native agents, web workers, model registry entries, SDK packages, and runtime bridges. Implements registry-first activation semantics: components only activate if they exist in the Atlas registry. Snapshot structure includes counts, inventory arrays, and SHA256 digest for change detection. Delta computation reveals drift between snapshots. Pack run lifecycle: preflight → native validation → freeze inventory → postflight → record lineage. Lineage structure tracks foundation_run, overlay_run, active_snapshot, and rollback_snapshot for complete audit trail.',
    axioms: [
      'Registrum primum, activatio secunda (Registry first, activation second)',
      'Inventarium unicum veritatis (Inventory is the single truth)',
      'Digestum determinat statum (Digest determines state)',
      'Delta mutationes revelat (Delta reveals changes)',
      'Lineage historiam servat (Lineage preserves history)',
      'Rollback semper possibilis (Rollback always possible)',
      'Profilus activationem gubernat (Profile governs activation)',
      'Snapshot immutabilis (Snapshot is immutable)',
      'Preflight et postflight custodiunt (Preflight and postflight guard)',
      'PRIMA CAUSA omnibus (PRIMA CAUSA for all)'
    ],
    tags: ['inventory', 'runtime', 'atlas-registry', 'pack-runs', 'lineage', 'rollback', 'drift-detection', 'snapshot', 'digest', 'registry-first'],
    stars: '★★★★★'
  },
  {
    rank: 35, id: 'PAPER-XXXVIII', number: 'Paper XXXVIII — PROT-160',
    latinTitle: 'De Centro Imperii',
    englishTitle: 'On the Command Center: Persistent Runtime Control and Telemetry',
    domain: 'command-center',
    engine: 'command-center.html + organism-bridge.js (organism-cli/web/)',
    scc: 4.81, phiOptimal: true,
    abstract: 'Establishes the Command Center — a persistent browser-based control interface for real-time organism monitoring, worker management, and incident response. Features: live status panel (worker count, heartbeat rate, coherence score, doctrine status), worker control grid (pause/resume/stop/start), incident log (real-time stream), pack status panel (profile, snapshot, digest, tokens), drift alerts (worker count, digest, heartbeat, coherence). Organism bridge enhancements: Atlas runtime config loading, profile application API, worker control actions, foundational shutdown guards (organism-bridge, engine-worker, protocol-worker, memory-worker cannot be stopped), incident telemetry. Deployed to ICP canisters via docs/ mirror.',
    axioms: [
      'Centrum imperat, non servit (The center commands, does not serve)',
      'Visibilitas totalis (Total visibility)',
      'Telemetria continua (Continuous telemetry)',
      'Incidentia archivatur (Incidents are archived)',
      'Profilus mutabilis (Profile is changeable)',
      'Fundamentum immobile (Foundation is immovable)',
      'Rollback semper possibilis (Rollback always possible)',
      'Drift detectatur (Drift is detected)',
      'Cohaerentia mensuratur (Coherence is measured)',
      'PRIMA CAUSA omnibus (PRIMA CAUSA for all)'
    ],
    tags: ['command-center', 'telemetry', 'monitoring', 'worker-control', 'incident-response', 'drift-detection', 'profile-switching', 'organism-bridge', 'real-time', 'sovereign-control'],
    stars: '★★★★★'
  },
];
...
function handleManifest() {
  ...
  return {
    ...
    substrate: 'All 21 papers are φ-optimal (SCC ≥ φ² ≈ 2.618). Their combined epistemic weight ' +
