...
@dataclass
class OrganismContract:
    """SDK contract for a provisioned intelligence organism."""
...
class ResearchLab:
    ...
    def __init__(self, name: str = "LABORATORIUM SOVEREIGNUS") -> None:
        ...
        self.organism_contracts: Dict[str, OrganismContract] = {}
    ...
    def create_organism_from_url(
        ...
        """Create one live organism from a target URL."""
        parsed = urlparse(website_url)
        if not parsed.scheme or not parsed.netloc:
            raise ValueError("website_url must be a valid absolute URL")

        organism_id = f"ORG-WEB-{len(self.organisms) + 1:04d}"
        lane_id = f"lane://{canonical_target.lower().replace('_', '-')}"
        contract_id = f"CONTRACT-{organism_id}"
        organism = {
        ...
        self.organism_contracts[contract_id] = OrganismContract(
        ...
        return {**organism, "contract_id": contract_id}

    def create_organism_from_domain(
        self,
        domain: str,
        canonical_target: str,
        domain_pack: Optional[str] = None,
    ) -> Dict[str, Any]:
        """Create one live organism from domain with lane/domain inference."""
        website_url = domain if domain.startswith(("https://", "http://")) else f"https://{domain}"
        parsed = urlparse(website_url)
        website_name = parsed.netloc or domain
        return self.create_organism_from_url(
            website_name=website_name,
            website_url=website_url,
            canonical_target=canonical_target,
            domain_pack=domain_pack or canonical_target.lower(),
        )

    def list_organism_contracts(self) -> List[Dict[str, Any]]:
        """List universal intelligence contracts for active organisms."""
        return [
            {
            ...
            for c in self.organism_contracts.values()
        ]
