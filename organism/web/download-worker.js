 ...
 * L-130 COMPLIANT — Every zip traces to primitive φ
 */
...
function buildExtensionZip(ext) {
  ...
  // popup.html
  if (ext.popup) {
    files.push({ name: 'popup.html', data: strToBytes(ext.popup) });
  }
  ...
  // styles.css
  if (ext.styles) {
    files.push({ name: 'styles.css', data: strToBytes(ext.styles) });
  }
  ...
  // README.md
  if (ext.readme) {
    files.push({ name: 'README.md', data: strToBytes(ext.readme) });
  }

  // LICENSE
  if (ext.license) {
    files.push({ name: 'LICENSE', data: strToBytes(ext.license) });
  }
